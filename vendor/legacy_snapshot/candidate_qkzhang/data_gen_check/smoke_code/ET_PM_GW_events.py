...
# 【新增】全局保存路径配置
# ==========================================
SAVE_DIR = "/root/autodl-tmp/qkzhang/data_gen_check/smoke_data/ET_PM_data"
...
for i in range(len(source_test_1)):
    for j in range(m):
        dL_test_1_ij = d_L_test_1[i][j]
        if dL_test_1_ij <= 0:
            raise ValueError(f"Non-positive test image 1 luminosity_distance: {dL_test_1_ij}")
        z_test_1 = luminosity_distance_to_redshift(dL_test_1_ij, cosmology=cosmo)
        para_1 = pd.DataFrame([{
            'luminosity_distance': dL_test_1_ij,
            'mass_1': source_test_1.iloc[i]['mass_1_source']*(1+z_test_1),
            'mass_2': source_test_1.iloc[i]['mass_2_source']*(1+z_test_1),
        ...
        dL_test_2_ij = d_L_test_2[i][j]
        if dL_test_2_ij <= 0:
            raise ValueError(f"Non-positive test image 2 luminosity_distance: {dL_test_2_ij}")
        z_test_2 = luminosity_distance_to_redshift(dL_test_2_ij, cosmology=cosmo)
        para_2 = pd.DataFrame([{
            'luminosity_distance': dL_test_2_ij,
            'mass_1': source_test_2.iloc[i]['mass_1_source']*(1+z_test_2),
            'mass_2': source_test_2.iloc[i]['mass_2_source']*(1+z_test_2),
...
def lens_PM_ampfac(mu_0, mu_1, t_d, frequencies, which_image=None):
    ...
    if which_image is None:
        ...
        i = which_image
        # For per-image event generation, catalog-level arrival time is already encoded
        # by geocent_time_2 = geocent_time_1 + t_d. Therefore, the per-image waveform
        # keeps magnification and Morse phase only, and does not apply an additional
        # exp(-2*pi*i*f*Delta_t) phase shift.
...
for i in range(len(lensed_source_params)//2):
    ...
    h_t_roll = shift_with_zeros(h_t, shift)
    h_t_white_roll = shift_with_zeros(h_t_white, shift_white)
...
final_snr_1  = os.path.join(SAVE_DIR, "PM_optimal_SNR_1.npy")
...
final_snr_2  = os.path.join(SAVE_DIR, "PM_optimal_SNR_2.npy")
...
test_final_snr_1  = os.path.join(SAVE_DIR, "test_PM_optimal_SNR_1.npy")
...
test_final_snr_2  = os.path.join(SAVE_DIR, "test_PM_optimal_SNR_2.npy")
