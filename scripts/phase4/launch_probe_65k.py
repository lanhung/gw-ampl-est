#!/usr/bin/env python3
"""Prepare the 65k rung once, then launch its three frozen seeds."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(gpu_indices) != 3 or len(set(gpu_indices)) != 3 or not all(
        ...
        raise ValueError("65k launcher requires three distinct numeric GPUs")
    script = arguments.root / "scripts/phase4/run_probe_65k.py"
    common = [
        sys.executable,
        str(script),
        "--root",
    ...
    launcher_results = arguments.output_root / "launcher-results"
    subprocess.run(
        common
        + [
            "--seed",
            ...
            "--prepare-rung-only",
            "--result",
            str(launcher_results / "rung-65536-preparation.json"),
        ],
        check=True,
    )
    log_root = arguments.output_root / "logs" / "rung-65536"
    log_root.mkdir(parents=True, exist_ok=True)
    processes = []
    streams = []
    for seed, gpu_index in zip((0, 1, 2), gpu_indices):
        result_path = launcher_results / f"rung-65536-seed-{seed}.json"
    ...
    summary = {
        "status": (
            "completed_all_three_65k_seed_processes"
            if all(item["return_code"] == 0 for item in return_codes.values())
            else "one_or_more_65k_seed_processes_failed"
        ),
        "rung": 65536,
    ...
    summary_path = launcher_results / "rung-65536-summary.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    summary_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    print(json.dumps(summary, indent=2, sort_keys=True))
    return 0 if summary["status"] == "completed_all_three_65k_seed_processes" else 1
