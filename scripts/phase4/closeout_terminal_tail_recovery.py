#!/usr/bin/env python3
"""Independently close out the atomically published parallel tail recovery."""
...
from scripts.phase4.run_terminal_tail_parallel_recovery import (
    AUTHORIZATION,
    BASE_CONFIG_HASH,
    GENERATOR_COMMIT,
    TRAIN_DATASET_ID,
...
def validate_tail_recovery_execution_result(
    root: Path,
    config: Mapping[str, Any],
    result: Mapping[str, Any],
) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
    """Validate recovery identities, counts, and closed safety boundaries."""
    ...
    if not (
        ...
        and frozen.get("base_configuration_hash") == configuration_hash(config)
        and orchestration.get("git_commit") == orchestration_commit
        and int(execution.get("worker_processes", -1)) == 32
        and int(execution.get("total_shard_count", -1)) == 128
        and int(execution.get("total_accepted_pairs", -1)) == TAIL_COUNT
        and flags.get("worker_64_authorized") is False
        ...
        raise ValueError("terminal recovery authorization changed")
    ...
    if any(result.get(key) != value for key, value in expected.items()):
        raise ValueError("terminal recovery official identity mismatch")
    if (
        result.get("status"),
        result.get("phase"),
        ...
        result.get("stopped_tail_evidence_reused"),
        ...
        "passed",
        "4-terminal-tail-parallel-recovery",
        GENERATOR_COMMIT,
        TRAIN_PARENT_ID,
        TRAIN_DATASET_ID,
        BASE_CONFIG_HASH,
        ...
        raise ValueError("terminal recovery count or safety contract failed")
    ...
    if int(result.get("remaining_free_bytes", -1)) < minimum_free:
        raise ValueError("terminal recovery recorded insufficient free space")
    return expected, authorization


def closeout_tail_recovery_publication(
    root: Path,
    *,
    config_path: str,
    ...
    """Resolve every parent and independently replay the two large tree hashes."""

    config = load_terminal_131k_contract(root, config_path)
    execution = _load_json(execution_result_path)
    identities, authorization = validate_tail_recovery_execution_result(
    ...
    if publication.combined_manifest_sha256 != execution["combined_manifest_sha256"]:
        raise ValueError("terminal recovery combined manifest mismatch")
    ...
    if recompute_tree:
        ...
        if (
            ...
            or tail_tree
            != execution.get("development_tail_publication_tree_sha256")
            or tail_bytes
            != int(execution.get("development_tail_publication_bytes", -1))
        ):
            raise ValueError("terminal recovery independent tree checksum failed")
    ...
    if observed_free < minimum_free:
        raise ValueError("terminal recovery closeout current free-space gate failed")
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = closeout_tail_recovery_publication(
        args.root,
